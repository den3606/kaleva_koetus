...
ascension.level = 1
ascension.description = "$kaleva_koetus_description_b" .. ascension.level
ascension.specification = "$kaleva_koetus_specification_b" .. ascension.level

-- local log = Logger:new("b1.lua")

local hp_multiplier = 2.5
local b1_enemy_tag = LevelTags.B1 .. EventTypes.ENEMY_POST_SPAWN
...
function ascension:on_enemy_post_spawn(entity_id, _, _)
  if EntityHasTag(entity_id, b1_enemy_tag) then
  ...
  EntityAddTag(entity_id, b1_enemy_tag)
