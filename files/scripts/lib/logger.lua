local existing = rawget(_G, "KalevaLogger")
if existing then
  return existing
end
...
function Logger:set_level(level)
  ...
  if normalized and self._levels[normalized] then
    ...
    print(string.format("%s[WARN][Logger] Unknown log level '%s', fallback to %s", self._prefix, tostring(level), self._level))
  end
end
