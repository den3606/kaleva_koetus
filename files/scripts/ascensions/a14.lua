...
function ascension:on_gold_spawn(entity_id)
  -- log:debug("on_gold_spawn")
  local lifetime_components = EntityGetComponent(entity_id, "LifetimeComponent")
  if lifetime_components ~= nil then
    for _, lifetime_component_id in ipairs(lifetime_components) do
      ...
      EntityRemoveComponent(entity_id, lifetime_component_id)
      local _ = EntityAddComponent2(entity_id, "LifetimeComponent", {
